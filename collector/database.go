...
func (c *databaseCollector) Collect(ch chan<- prometheus.Metric, name string, db client.Database) {
	ch <- prometheus.MustNewConstMetric(c.abandonedSeqs, prometheus.CounterValue, float64(db.AbandonedSeqs), name)
	...
	ch <- prometheus.MustNewConstMetric(c.docReadsBytesBlip, prometheus.CounterValue, float64(db.DocReadsBytesBlip), name)
	ch <- prometheus.MustNewConstMetric(c.docWritesBytes, prometheus.CounterValue, float64(db.DocWritesBytes), name)
	ch <- prometheus.MustNewConstMetric(c.docWritesBytesBlip, prometheus.CounterValue, float64(db.DocWritesBytesBlip), name)
	ch <- prometheus.MustNewConstMetric(c.numDocReadsBlip, prometheus.CounterValue, float64(db.NumDocReadsBlip), name)
	...
	ch <- prometheus.MustNewConstMetric(c.warnChannelsPerDocCount, prometheus.CounterValue, float64(db.WarnChannelsPerDocCount), name)
	ch <- prometheus.MustNewConstMetric(c.warnGrantsPerDocCount, prometheus.CounterValue, float64(db.WarnGrantsPerDocCount), name)
	ch <- prometheus.MustNewConstMetric(c.warnXattrSizeCount, prometheus.CounterValue, float64(db.WarnXattrSizeCount), name)
}
