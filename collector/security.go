...
func (c *securityCollector) Collect(ch chan<- prometheus.Metric, name string, sec client.Security) {
	...
	ch <- prometheus.MustNewConstMetric(c.numAccessErrors, prometheus.GaugeValue, float64(sec.NumAccessErrors), name)
	ch <- prometheus.MustNewConstMetric(c.numDocsRejected, prometheus.GaugeValue, float64(sec.NumDocsRejected), name)
