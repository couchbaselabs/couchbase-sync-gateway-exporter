...
func (c *pullCollector) Collect(ch chan<- prometheus.Metric, name string, pull client.CblReplicationPull) {
	...
	ch <- prometheus.MustNewConstMetric(c.numPullReplSinceZero, prometheus.CounterValue, pull.NumPullReplSinceZero, name)
	ch <- prometheus.MustNewConstMetric(c.numPullReplTotalContinuous, prometheus.GaugeValue, pull.NumPullReplTotalContinuous, name)
	ch <- prometheus.MustNewConstMetric(c.numPullReplTotalOneShot, prometheus.GaugeValue, pull.NumPullReplTotalOneShot, name)
	ch <- prometheus.MustNewConstMetric(c.requestChangesCount, prometheus.CounterValue, pull.RequestChangesCount, name)
	ch <- prometheus.MustNewConstMetric(c.requestChangesTime, prometheus.CounterValue, pull.RequestChangesTime, name)
	ch <- prometheus.MustNewConstMetric(c.revProcessingTime, prometheus.GaugeValue, pull.RevProcessingTime, name)
	ch <- prometheus.MustNewConstMetric(c.revSendCount, prometheus.CounterValue, pull.RevSendCount, name)
	ch <- prometheus.MustNewConstMetric(c.revSendLatency, prometheus.CounterValue, pull.RevSendLatency, name)
}
