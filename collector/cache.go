...
func (c *cacheCollector) Collect(ch chan<- prometheus.Metric, name string, cache client.Cache) {
	...
	ch <- prometheus.MustNewConstMetric(c.highSeqCached, prometheus.GaugeValue, cache.HighSeqCached, name)
	ch <- prometheus.MustNewConstMetric(c.highSeqStable, prometheus.GaugeValue, cache.HighSeqStable, name)
